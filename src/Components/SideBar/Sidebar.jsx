...
const Sidebar = ({routes, collapsed, setCollapsed}) => {
    ...
    return (
        <div className="bg-white mt-6 rounded-r-2xl h-[1000px] mr-6">
            <Sider
                ref={siderRef}
                trigger={null}
                collapsible
            ...
            >
                <Button
                    ...
                    icon={
                        <div
                            className="flex justify-center items-center bg-Golden w-8 h-8 rounded-lg transition-all duration-700">
                            {collapsed ? (
                                <Icon
                ...
                <div className="p-4 flex justify-center items-center bg-white rounded-t-2xl mb-5">
                    <img src={logo} alt="logo"
                         className={`${collapsed ? "h-6 w-6 ml-2 translate-x-0" : "h-28 w-26 hover:filter drop-shadow-[0_4px_6px_rgba(0,0,0,0.4)]"}`}/>
                </div>
                <div className="w-full h-10 flex flex-row items-center justify-center px-4 mt-2">
                    {collapsed ? (
                        <Icon icon="hugeicons:search-01" width="20" height="20" style={{color: "#3A3541"}}/>
                    ) : (
                        <Input
                            placeholder="Search ..."
                            prefix={<Icon icon="hugeicons:search-01" width="10" height="10"
                                          style={{color: "#3A3541"}}/>}
                            className="ml-4 mb-4"
                        />
                    )}
